...
<div>
    {#if vertical}
        <svg {height} viewBox="0 0 51 114" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g id="battery">
                <rect id="battery-fill" y={107 - percentage} width="51" height={7 + percentage} rx="8" {fill}/>
                ...
                <text id="battery-percentage" x="25" y="60" fill="white"
                      class="font-number {height > 50 ? `text-lg` : `text-sm`}" text-anchor="middle">{percentage}%</text>
            </g>
        </svg>
    {:else}
        <svg {height} viewBox="0 0 115 52" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g id="battery">
                <rect id="battery-fill" x="107.5" y={100-percentage} width="51" height={7.5 + percentage} rx="8" transform="rotate(90 107.5 0.5)"
                      {fill}/>
                ...
                <text id="battery-percentage" x="55" y="32" fill="white"
                      class="font-number {height > 30 ? 'text-lg' : 'text-2xl'}" text-anchor="middle">{percentage}%</text>
            </g>
        </svg>
    {/if}
</div>